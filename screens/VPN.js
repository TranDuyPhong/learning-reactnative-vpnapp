...
class VPN extends Component {
    ...
    renderServer() {
        ...
        const condition = server || automatic;
        return (
            <Block flex={false} row center middle>
                <Image source={condition.icon} />
                <Text margin={[0, 10, 0, 20]} center>{condition.name}</Text>
            ...
            <Modal visible={show} animationType='fade' transparent>
                <Block bottom color={rgba(COLORS.gray, 0.2)}>
                    <Block flex={false} white middle padding={[SIZES.padding, 0]}>
                        ...
                        <ScrollView>
                            {servers.map(item => {
                                const isConnected = condition.name === item.name;
                ...
                <Block center middle flex={false}>
                    <Block 
                        ...
                        <Block 
                            ...
                            radius={SIZES.base} 
                            style={styles.status}></Block>
                    </Block>
                    <Image source={icons[connected ? 'online' : 'offline']} style={styles.image} />
                    <Button
                        theme={theme} 
                        outlined={connected} 
                        style={[styles.connect, connected && styles.connected]} 
                        onPress={() => this.handleConnect()}>
                        <Text white={!connected} caption center bold margin={[SIZES.padding / 2, 0]}>
                            {
